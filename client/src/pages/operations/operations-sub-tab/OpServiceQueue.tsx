...
export default function OpServiceQueue() {
  ...
  useEffect(() => {
    ...
    toast.info("Line item updated", { 
      id: "line-item-update",
      description: "Queue updated with latest changes"
    });
          ...
          {isLoading ? (
            ...
            <TableRow>
              <TableCell colSpan={12} className="text-center py-8 text-gray-500">
                {searchTerm || filterPriority !== 'all' ? 'No items match your filters' : 'No items in queue'}
              </TableCell>
            </TableRow>
          ) : (
      ...
      <div className="op-below-container flex flex-wrap justify-between items-center gap-3 mt-2">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              className="op-btn-rd op-btn text-white bg-[#1447E6] button-md disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#1239CC] transition-colors flex items-center gap-1"
              disabled={selected.length === 0}
              onClick={() => setModalOpen(true)}
              title={selected.length === 0 ? "Select items to mark as ready" : `Mark ${selected.length} items as ready`}
            >
              <CheckCircle2 className="w-4 h-4" />
              <span className="text-sm font-medium hidden md:inline">Mark as Ready</span>
              <span className="text-sm font-medium md:hidden">Ready</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <MarkAsReadyModal
        ...
        onConfirm={async () => {
          try {
            ...
            setSelected([]);
            setModalOpen(false);
            toast.success("Selected items marked as ready for delivery!");
          } catch (error) {
