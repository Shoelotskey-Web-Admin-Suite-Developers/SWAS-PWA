...
export default function OpReturnBranch({ readOnly = false }) {
  ...
  useEffect(() => {
    ...
    toast.info("Line item updated", { 
      id: "line-item-update",
      description: "Return to branch updated with latest changes"
    });

    if (changes.operationType === "insert" || changes.operationType === "replace") {
      // Handle inserts or full replacements
      if (changes.fullDocument && changes.fullDocument.current_status === "Returning to Branch") {
      ...
      if (changes.documentKey && changes.documentKey._id) {
        ...
        if (changes.fullDocument) {
          ...
          if (item.current_status === "Returning to Branch") {
  ...
  const toggleSelectAll = () => {
    if (readOnly) return; // Don't toggle if readOnly
          ...
          {isLoading ? (
            ...
            <TableRow>
              <TableCell colSpan={12} className="text-center py-8 text-gray-500">
                {searchTerm || filterPriority !== 'all' ? 'No items match your filters' : 'No items returning to branch'}
              </TableCell>
            </TableRow>
          ) : (
            filteredRows.map((row, index) => (
              <React.Fragment key={row.lineItemId}>
                ...
                >
                  ...
                  <TableCell className={`op-body-customer ${getUpdateColor(row.updated)}`}><small>{row.customerName || row.customerId}</small></TableCell>
